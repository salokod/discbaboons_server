...
const forgotPasswordController = async (req, res) => {
  ...
  try {
    ...
    if (userExist.Count === 1) {
      ...
      let TTL_DELTA = 60 * 60; //Keep records for 1 hour
    ...
    return res.status(200).json({ message: "An email will be sent to the email address on file, if username exists...", urlUuid: urlUuid });
  } catch (error) {
